...
# In[3]:
def kaixiangbaogao(text, addr, num):
    # In[5 ]: 制作物资开箱报告
    ...
    for para in doc.paragraphs:
        ...
        for run in para.runs:
            if hetong_placeholder in run.text:
                # 用具体的名字替换占位符
                run.text = run.text.replace(hetong_placeholder, hetong_number)

            if dingdan_placeholder in run.text:
                run.text = run.text.replace(dingdan_placeholder, code)

            if reason_placeholder in run.text:
                lingyong_reason = f"此订单对应的设备发货到{addr}第{num}批，部件组装后对应{text}。设备及配置清单请见订单以及到货证明附件。"
...
for i, s in df.iloc[218:220].iterrows():  # 这个序号要比表格中的index要少2，比方说江苏南通5的index是219，那么这里应该写为217:218
    ...
    final_text = f"领用后，部件组装后对应{text}。上电、调试，测试合格后，按照合同号：{c2}发货到{addr}第{num}批。"
    sht["B6"].value = final_text
    wb.save(f"物资出库申请-{code}.xlsx")
    print("保存到: ", f"物资出库申请-{code}.xlsx")
    kaixiangbaogao(text, addr, num)
